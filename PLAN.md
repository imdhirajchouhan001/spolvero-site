...
**Every article has its own photograph; no two guides share one.** Set 19
September 2026. Each article names its photo with `image:` in its front matter,
a key into `src/data/photos.ts`. Borrowing the tool's photo was tried first and
looked monotonous — seven webcam guides with the same picture — so eleven were
added, chosen on unsplash.com with the free-licence filter on (which excludes
Unsplash+). Five articles kept an existing photo that suited them best. A new
article without `image:` still falls back to its tool's photo, so pick one for
it when it is written, and check it is not already in use.
