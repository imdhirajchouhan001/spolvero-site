...
const allProducts: Product[] = [
  ...
  },
  {
    slug: "flash",
    name: "Flash",
    storeName: "Flash: Torch and Screen Light",
    seoTitle: "Flash — A Torch App That Asks for No Permissions at All",
    kind: "Light tool",
    status: "soon",
    platform: "Android and iOS",
    tagline: "Every kind of light. Nothing asked in return.",
    summary:
    ...
    features: [
      ...
      { icon: "message", title: "Morse and signalling", body: "Type a message and the light sends it. Useful far less often than it is satisfying, but genuinely useful when it is." },
      { icon: "monitor", title: "Where you actually reach for it", body: "A Quick Settings tile on Android and a Control Centre control on iOS, so the light is one swipe away rather than an app launch." },
    ],
    promise: {
      title: "Zero permissions, and that is checkable",
      body: "This is the easiest privacy claim on the site to verify, because the app declares nothing to verify against.",
      points: [
        "No permissions declared, so none can be requested",
        "No camera is opened — the LED is driven directly",
        "No account, no analytics, no network calls",
        "Torch state is read from the system, never cached, so the app always agrees with Quick Settings",
        "A phone with no LED opens in screen mode rather than offering a control that cannot work",
      ],
    },
    useCases: [
      { who: "Anyone who read the permissions", when: "You noticed a torch app wanted camera access and wondered why a light needs to see." },
      { who: "Someone signalling", when: "Morse, or a repeating flash, when you need to be found rather than to see." },
      { who: "Reading in the dark", when: "A warm, dim screen light is kinder at 2am than an LED pointed at the ceiling." },
      { who: "People who want it fast", when: "A tile or Control Centre control, rather than unlocking and hunting for an icon." },
    ],
    versus: {
      title: "How this differs from most torch apps",
      rows: [
        { them: "Asks for camera permission to reach the LED", us: "Declares no permissions at all" },
        { them: "Shows full-screen ads when you need light in a hurry", us: "Opens straight into the light" },
        { them: "Caches the torch state and disagrees with Quick Settings", us: "Reads the state from the system, so the two always match" },
        { them: "Offers a strobe on a phone with no LED", us: "Probes what the hardware can do and opens in screen mode instead" },
        { them: "Android only, or iOS only", us: "The same app on both" },
      ],
    },
    gettingStarted: [
      { name: "Open it", text: "The light is the first screen. There is no sign-up, no tour and no permission prompt." },
      { name: "Pick your light", text: "The LED torch, or the screen as a lamp in whatever colour and brightness suits the room." },
      { name: "Put it where you reach", text: "Add the Quick Settings tile on Android or the Control Centre control on iOS, so it is one swipe away." },
    ],
    alsoDoes: [
      "Adjustable torch strength where the hardware supports it",
      "Screen light with colour and brightness control",
      "Morse from typed text",
      "Strobe with adjustable rate",
      "Home screen widgets and launcher shortcuts",
      "An auto-off timer so it cannot drain the battery overnight",
    ],
    readingTags: ["Privacy"],
    faqs: [
      { q: "Why do other torch apps want camera permission?", a: "Because reaching the LED through a camera session is the easier route to write. Both Android and iOS also expose the torch directly — CameraManager.setTorchMode and AVCaptureDevice.torchMode — and that path needs no permission. Flash uses the direct one." },
      { q: "Does it really request nothing?", a: "Nothing. There are no permissions declared in the manifest or the Info.plist, so there is nothing for the system to prompt about. That is a claim you can check on the store listing before installing." },
      { q: "What if my phone has no LED?", a: "It checks at startup and opens in screen-light mode instead, rather than showing a torch control that would do nothing." },
      { q: "Will it work with the Quick Settings tile?", a: "That is the intended way to use it — a tile on Android and a Control Centre control on iOS, so the light is reachable without opening the app." },
      { q: "When is it out?", a: "It is in development. The light itself works on real hardware; the tile, widgets and settings are still being built. Join the list and we will email you when it is on the stores." },
    ],
  },
  {
    slug: "gst-calculator",
